...
function getAllUsers() {
  try {
    ...
    return [...DEMO_USERS, ...(s ? JSON.parse(s) : [])];
  } catch { return [...DEMO_USERS]; }
}
function authLogin(email, password) {
  const found = getAllUsers().find(u => u.email === email && u.password === password);
  if (found) { localStorage.setItem(AUTH_KEY, JSON.stringify(found.user)); return found.user; }
  return null;
}
function authRegister(email, password, name) {
  if (getAllUsers().find(u => u.email === email)) return { success: false, message: 'Este correo ya está registrado' };
  const user = { id: Date.now().toString(), name, email, role: 'cliente' };
  try {
    const s = localStorage.getItem(REG_KEY);
    const reg = s ? JSON.parse(s) : [];
    reg.push({ email, password, user });
    localStorage.setItem(REG_KEY, JSON.stringify(reg));
    localStorage.setItem(AUTH_KEY, JSON.stringify(user));
    return { success: true, user };
  } catch { return { success: false, message: 'Error al registrar' }; }
}
function authLogout() { localStorage.removeItem(AUTH_KEY); }
